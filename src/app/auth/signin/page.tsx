...
import { Eye, EyeOff, Mail, Lock, ArrowLeft, Heart, Brain } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.data && !result.error) {
        ...
        toast.error(typeof result.error === 'string' ? result.error : (result.error as any)?.message || 'Sign in failed');
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary-50 via-background to-secondary-50 dark:from-primary-950 dark:via-background dark:to-secondary-950 flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        {/* AI Health Coach Preview */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.4 }}
          className="mt-8 p-4 rounded-lg bg-gradient-to-r from-success-50 to-primary-50 dark:from-success-950 dark:to-primary-950 border border-success-200 dark:border-success-800"
        >
          <div className="flex items-start space-x-3">
            <Brain className="w-5 h-5 text-success-600 mt-0.5" />
            <div>
              <p className="text-sm font-medium text-foreground">AI Health Coach</p>
              <p className="text-xs text-muted-foreground">
                Get personalized health advice, meal recommendations, and wellness tips powered by advanced AI.
              </p>
            </div>
          </div>
        </motion.div>
